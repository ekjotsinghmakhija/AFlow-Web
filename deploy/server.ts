...
const createServer = async (req: Request) => {
  ...
  if (['/after-payment', '/login', '/as-template', '/app', '/accept-invitation', '/import'].some(item => reqUrl.pathname.startsWith(item))) {
    ...
    if (reqUrl.pathname === '/after-payment') {
      title = 'Payment Success | AFlow';
      description = 'Payment success on AFlow';
    }

    if (reqUrl.pathname === '/login') {
      title = 'Login | AFlow';
      description = 'Login to AFlow';
    }
  ...
  if (req.method === 'GET') {
    ...
    const description = 'Write, share, and publish docs quickly on AFlow.\nGet started for free.';
    let title = 'AFlow';
    ...
    setOrUpdateMetaTag($, 'meta[property="og:site_name"]', 'property', 'AFlow');
    ...
    setOrUpdateMetaTag($, 'meta[name="twitter:site"]', 'name', '@aflow');
